...
class Neuro_gambit(nn.Module):
    ...
    def forward_str_input(self, positions, player):
    ...
    def forward_chess_board_input(self, board, player):
...
def get_move_probability(move, tensor_tuple):
...
def get_best_move(model, board, player):
    if type(board) != chess.Board:
        board = matrix_to_board(linear_to_matrix(board), player)
